...
const LoginForm = ({ show, onClose }) => {
  ...
  const handleSignUp = (e) => {
    ...
    axios
      ...
      .then(() => {
        return axios.post(`https://localhost:7148/api/LoginJWT/Log-in-Hash`, params);
      })
      .then((response) => {
        Auth_JWT.login(response.data.token);
        setLoading(false);
          ...
          >
            ...
            <div className="social-icons">
              <GoogleOAuthProvider clientId="191234775662-mda3goonrsk2g68bu3dknkpkgrh34431.apps.googleusercontent.com">
                <GoogleLogin
                  onSuccess={(credentialResponse) => {
                    ...
                    handleSignUp_WithGoogle(jwtDecode(credentialResponse.credential));
                    onClose();
                    window.location.reload();
                  }}
                  onError={() => {
              ...
              <a href="#" className="icon" style={{ border: "none" }}>
                <img src={fb} style={{ width: "70%", height: "70%" }} alt="fb"></img>
              </a>
              <a href="#" className="icon" style={{ border: "none" }}>
                <img src={github} style={{ width: "120%", height: "120%" }} alt="github"></img>
              </a>
            </div>
            <span>或使用電子郵件註冊</span>
            <input
